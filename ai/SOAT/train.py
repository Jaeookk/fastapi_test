...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device):
    ...
    for idx in pbar:
        ...
        fake_img, outputs = generator(noise) 
        if args.augment:
            fake_img, _ = augment(fake_img, ada_aug_p)
        ...
        if get_rank() == 0:
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    utils.save_image(
                        sample,
                        f"sample_{args.dataset}/{str(i).zfill(6)}.png",
            ...
            if i % 2000 == 0:
                torch.save(
                    {
                        "g": g_module.cpu().state_dict(),
                        "d": d_module.cpu().state_dict(),
                        "g_ema": g_ema.cpu().state_dict(),
                        "g_optim": g_optim.state_dict(),
                        "d_optim": d_optim.state_dict(),
                        "args": args,
                        "ada_aug_p": ada_aug_p,
                    },
                    f"checkpoint_{args.dataset}/{str(i).zfill(6)}.pt",
                )
                g_module.to(device)
                d_module.to(device)
                g_ema.to(device)
...
if __name__ == "__main__":
    ...
    if args.ckpt is not None:
        ...
        # freeze mapping layers
        for name, param in generator.named_parameters():
            if param.requires_grad and 'style' in name:
                param.requires_grad = False
            if param.requires_grad and 'modulation' in name:
                param.requires_grad = False

    g_ema_base = deepcopy(g_ema).eval().requires_grad_(False)
