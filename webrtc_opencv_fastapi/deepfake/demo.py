...
def load_checkpoints(config_path, checkpoint_path, device):
    ...
    dense_motion_network = DenseMotionNetwork(
        **config["model_params"]["common_params"], **config["model_params"]["dense_motion_params"]
    )
    avd_network = AVDNetwork(
        num_tps=config["model_params"]["common_params"]["num_tps"], **config["model_params"]["avd_network_params"]
    )
...
def make_animation(
    ...
    with torch.no_grad():
        ...
        driving = (
            torch.tensor(np.array(driving_video)[np.newaxis].astype(np.float32)).permute(0, 4, 1, 2, 3).to(device)
        )
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            if mode == "standard":
                ...
                kp_norm = relative_kp(
                    kp_source=kp_source, kp_driving=kp_driving, kp_driving_initial=kp_driving_initial
                )
            elif mode == "avd":
                kp_norm = avd_network(kp_source, kp_driving)
            dense_motion = dense_motion_network(
                source_image=source, kp_driving=kp_norm, kp_source=kp_source, bg_param=None, dropout_flag=False
            )
